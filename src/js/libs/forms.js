...
function forms(){
	...
	//FIELDS
	$('input,textarea').focus(function(){       //берем все элементы .input или .textarea и при фокусе
		if($(this).val() == $(this).attr('data-value')){  // если у этого элемента val (val - достаем значение) == есть атрибут "data-value", то
			...
			if($(this).attr('data-type')=='pass'){        // если у этого поля есть аттрибут data-value=pass, то 
